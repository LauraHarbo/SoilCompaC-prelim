...
Not denmark
```{r}
df.coords_germany <- read_csv(here("Input/Coordinates/Germany_coordinates.csv"))%>%select(-c(SiteNo))
df.coords_belgium <- read_csv(here("Input/Coordinates/Belgium_coordinates.csv"))%>%select(-c(SiteNo))
df.coords_ireland <- read_csv(here("Input/Coordinates/Ireland_coordinates.csv"))%>%select(-c(SiteNo))
df.coords_france <- read_csv(here("Input/Coordinates/France_coordinates.csv"))%>%select(-c(SiteNo))

df.coords <- rbind(df.coords_belgium, df.coords_germany, df.coords_ireland, df.coords_france)%>%
  distinct()
...
## Merge with data
```{r}
df.baselineinput <- read_csv(here("Derived/Data/20240830_allCountries_BaselineModelData_LSH.csv"))%>%
  left_join(df.coords, by=c("ID"))
...
## Add DK as a grid
As the coordinates from DK are not (currently?) available, the DK data is added as a grid (waffle plot) over the map. 
```{r}
...
## Export figure
```{r}
# ggsave("Derived/Figures/20241026_F1_LUmap2_bluepink5.png", plot=fig1, width=12, height=12, dpi=400)
```
...
```{r}
df.baseline_varImp0 <- read_csv(here("Derived/Data/20240830_baselinemodel_VarImp_LSH.csv")) 
...
fig2a <- ggplot(ImpData%>%filter(Importance > 5e-04), 
  ...
  scale_x_continuous(limits = c(0,0.014), expand = c(0, 0))
...
```{r}
# get data
df.pdp_oc <- read_csv(here("Derived/Data/20240830_baselinemodel_pdp_oc_LSH.csv"))

df.pdp_silt <- read_csv(here("Derived/Data/20240830_baselinemodel_pdp_silt_LSH.csv"))

df.pdp_clay <- read_csv(here("Derived/Data/20240830_baselinemodel_pdp_clay_LSH.csv"))

df.pdp_map <- read_csv(here("Derived/Data/20240830_baselinemodel_pdp_map_LSH.csv"))
```
...
fig2b <- ggplot(df.pdp_oc,
  ...
  scale_y_continuous(limits = c(1.25, 1.5), breaks =c(1.3, 1.4, 1.5))+
  annotate("rect", xmin = Bound.low, xmax = 10, ymin = 1.25, ymax = 1.5, alpha = .2, fill = "grey")+
  annotate("rect", xmin = 0, xmax =Bound.high, ymin = 1.25, ymax = 1.5, alpha = .2, fill = "grey")
...
fig2c <- ggplot(df.pdp_clay,
  ...
  scale_y_continuous(limits = c(1.25, 1.5), breaks =c(1.3, 1.4, 1.5))+
  annotate("rect", xmin = Bound.low, xmax = 93, ymin = 1.25, ymax = 1.5, alpha = .2, fill = "grey")+
  annotate("rect", xmin = 0, xmax =Bound.high, ymin = 1.25, ymax = 1.5, alpha = .2, fill = "grey")
...
fig2d <- ggplot(df.pdp_silt,
  ...
  # add "invisible" data as points to generate marginal density plot
  geom_point(data=df.baselineinput, aes(x = Silt, y = mean(BD)), shape=NA
             )+
  scale_color_manual(values=c("grey80"))+
    theme(legend.position = "none", 
          axis.title.x = element_text(size=12), 
          axis.title.y = element_text(size=10))+
  scale_y_continuous(limits = c(1.25, 1.5), breaks =c(1.3, 1.4, 1.5))+
  annotate("rect", xmin = Bound.low, xmax = 85, ymin = 1.25, ymax = 1.5, alpha = .2, fill = "grey")+
  annotate("rect", xmin = 0, xmax =Bound.high, ymin = 1.25, ymax = 1.5, alpha = .2, fill = "grey")
...
fig2e <- ggplot(df.pdp_map,
  ...
  scale_color_manual(values=c("grey80"))+
    theme(legend.position = "none", 
          ...
          axis.title.y = element_blank())+
  scale_y_continuous(limits = c(1.25, 1.5), breaks =c(1.3, 1.4, 1.5))+
  annotate("rect", xmin = Bound.low, xmax = 2200, ymin = 1.25, ymax = 1.5, alpha = .2, fill = "grey")+
  annotate("rect", xmin = min(df.baselineinput$MAP), xmax =Bound.high, ymin = 1.25, ymax = 1.5, alpha = .2, fill = "grey")
...
## Combine pdp figs
```{r}
fig2_pdp <- ggarrange(fig2b, fig2c, fig2d, fig2e, 
...
## Export figure
```{r}
# ggsave("Derived/Figures/20240830_F3_baselinemodel.png", plot=fig2, width=10, height=4, dpi=400)
```
...
var.list <- df.baseline_varImp0%>%filter(Importance > 5e-04)%>% arrange(desc(Importance))
...
```{r}
x_mean <- mean(df.5fold$BD)
y_mean <- mean(df.5fold$pred)

fig3a <- ggplot(df.5fold, 
       aes(x = BD, y = pred, shape=factor(fold)
           )
       )+
  ...
  labs(
    ...
    y = expression("Est. Glassland BD" ~ "(g cm" ^ {-3} * ")")
  )+
  ...
  geom_point(x=x_mean, y= y_mean, shape = 4, size=5, col="grey20", stroke = 1.5)

fig3a
```

```{r}
df.pred <- read_csv(here("Derived/Data/20240830_SubsoilCompaction_allCountries_LSH.csv"))%>%
...
## Export figure
```{r}
# ggsave("Derived/Figures/20241120_F3_5foldCV_results.png", plot=fig3, width=12, height=4, dpi=400)
```

```{r}
length(df.pred_AC$ID)
length(df.5fold$fold)
```
...
fig4a <- ggplot() + 
  ...
  #      aes(col=BD_diff_class), size=3)+
  # # Flanders
  # geom_sf(data=df.Predpoints%>%
  #           filter(AOA == 1, Survey == "CMON"), 
  #      aes(col=BD_diff_class), size=0.5)+
  ...
  guides(color = guide_legend(nrow=2, byrow=TRUE, override.aes = list(size=5), title.position="top", title.hjust = 0.5))+
...
fig4afull <- fig4afull.0 + annotation_custom(ggplotGrob(plot.pred.cmon),
                          ...
                          ymin = 3100597, ymax = 3558510 # 3508510
                          )
...
## NUTS2 map
```{r}
df.nuts <- read_csv(here("Derived/Data/20240708_SurveyData_geodata_LSH.csv"))%>%
  select(ID, NUTS2)%>%
  mutate(
...
## Export figure
```{r}
# ggsave("Derived/Figures/20240925_F4_Predmap_points_adjusted.png", plot=fig4afull, width=12, height=12, dpi=400)
```

```{r}
# ggsave("Derived/Figures/20240830_F4_Predmap_nuts1.png", plot=fig4.b, width=12, height=12, dpi=400)
```

```{r}
# ggsave("Derived/Figures/20240925_F4.png", plot=fig4, width=12, height=7.2, dpi=400)
```
...
## Export figure
```{r}
# ggsave("Derived/Figures/20241121_F.S2_histogram.png", plot=figS2, width=15, height=6, dpi=400)
```
...
# Posthoc model Fig. 5
```{r}
df.posthoc_varImp <- read_csv(here("Derived/Data/20240830_posthocModel_LSH.csv"))%>%
  # fix names
  mutate(
    Name = case_when(
      name == "OC" ~ "Organic C", 
...
## Export figure
```{r}
# ggsave("Derived/Figures/20240830_F5_VarImp_posthoc.png", plot=fig5, width=8, height=4.8, dpi=400)
```
...
df.twosidedpdp <- read_csv(here("Derived/Data/20240830_posthoc_2sidedpdp_texture_LSH.csv"))%>%
  mutate(
    Sand = 100 - Clay - Silt
  )
...
fig.6a <- ggplot(df.twosidedpdp, 
  ...
  scale_fill_gradient2(low = "#ffe2e3", mid = "#E7B9C2", high = "#9c3e5d", 
                       midpoint = 0.035, na.value = "grey50", 
...
## Export figure
```{r}
# ggsave("Derived/Figures/202401001_F6_TrianglePlot.png", plot=fig.6a, width=10, height=10, dpi=400)
```

## Univariate pdps
```{r}
df.ph_clay <- read_csv(here("Derived/Data/20240830_posthoc_pdp_clay_LSH.csv"))
...
```{r}
df.ph_silt <- read_csv(here("Derived/Data/20240830_posthoc_pdp_silt_LSH.csv"))
...
```{r}
df.ph_oc <- read_csv(here("Derived/Data/20240830_posthoc_pdp_oc_LSH.csv"))
...
fig6d <- ggplot(df.ph_oc,
       aes(x=OC, y=yhat, col="A"))+ # add color to control color of marginal density plot
  geom_segment(aes(x=0, y=0, xend = 2.5, yend = 0), col= "grey70", lty = "dashed", size=0.9)+
  geom_line(size=1.2, col="#cb627b" # col="#67a9b6"
            )+
  theme_classic(base_size = 15)+
  labs(
  ...
  annotate("rect", xmin = Bound.low, xmax = 2.5, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")+
  annotate("rect", xmin = 0, xmax =Bound.high, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")
...
```{r}
df.ph_map <- read_csv(here("Derived/Data/20240830_posthoc_pdp_map_LSH.csv"))
...
fig6e <- ggplot(df.ph_map,
  ...
  annotate("rect", xmin = Bound.low, xmax = 1850, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")+
  annotate("rect", xmin = 500, xmax =Bound.high, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")
...
```{r}
df.ph_mfs <- read_csv(here("Derived/Data/20240830_posthoc_pdp_mfs_LSH.csv"))
...
fig6f <- ggplot(df.ph_mfs,
  ...
  scale_y_continuous(limits = c(-0.03, 0.07))+
  #scale_x_continuous(limits = c(500, 1850))+
  annotate("rect", xmin = Bound.low, xmax = 300, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")+
  annotate("rect", xmin = 0, xmax =Bound.high, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")
...
```{r}
df.ph_mat <- read_csv(here("Derived/Data/20240830_posthoc_pdp_mat_LSH.csv"))
...
fig6g <- ggplot(df.ph_mat,
  ...
  annotate("rect", xmin = Bound.low, xmax = 16, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")+
  annotate("rect", xmin = 6, xmax =Bound.high, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")
...
```{r}
df.ph_ph <- read_csv(here("Derived/Data/20240830_posthoc_pdp_ph_LSH.csv"))
...
fig6h <- ggplot(df.ph_ph,
  ...
  annotate("rect", xmin = Bound.low, xmax = 9, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")+
  annotate("rect", xmin = 3.5, xmax =Bound.high, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")
...
```{r}
df.ph_till <- read_csv(here("Derived/Data/20240830_posthoc_pdp_till_LSH.csv"))
...
fig6i <- ggplot(df.ph_till,
  ...
  annotate("rect", xmin = Bound.low, xmax = 90, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")+
  annotate("rect", xmin = 20, xmax =Bound.high, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")
...
```{r}
df.ph_ps <- read_csv(here("Derived/Data/20240830_posthoc_pdp_ps_LSH.csv"))
...
fig6j <- ggplot(df.ph_ps,
  ...
  annotate("rect", xmin = Bound.low, xmax = 22, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")+
  annotate("rect", xmin = 0, xmax =Bound.high, ymin = -0.03, ymax = 0.07, alpha = .2, fill = "grey")
...
## Export figure
```{r}
# ggsave("Derived/Figures/20241126_F6_univariate.png", plot=fig6right, width=10, height=9, dpi=400)
```

## Export figure
```{r}
# ggsave("Derived/Figures/20240830_F6.png", plot=fig6, width=20, height=8, dpi=400)
```
...
```{r}
# get data
df.posthocinput <- read_csv(here("Derived/Data/20241025_posthocModelData_LSH.csv"))%>%
  # select vars
...
f7germany <- ggplot() + 
  ...
  #           filter(Survey == "RMQS"), 
  #      aes(col=Compacted, shape = Compacted),size=2)+
  # # Germany
  geom_sf(data=df.comppoints%>%
            filter(Survey == "BZE"),
...
f7a <- ggplot(df.f6, 
  ...
  # scale_fill_manual(values=c("#67a9b6", "#cb627b", "#9c3e5d"), breaks=c('Not compacted', 'Naturally compacted', 'Anthropologically compacted')
  #                   )+
  ...
  # xlab(expression("Proportion of annual cropland sites where PD > 1.71"))
...
f7alternative2 <- ggarrange(f7france, f7germany,# f7a,
                  ncol=2, nrow=1, common.legend = TRUE, legend="bottom", 
          labels = c("A", "B"),
          widths = c(1, 1),
          #heights = c(0.4, 1, 0.2),
...
```{r}
# ggsave("Derived/Figures/20241101_Fig7_withmap_v2.png", plot=f7alternative3, width=18, height=14, dpi=400)
```
...
```{r}
# ggsave("Derived/Figures/20241121_FigS4_withmap_.png", plot=f7alternative3_s, width=18, height=14, dpi=400)
```
...
```{r}
# ggsave("Derived/Figures/20241121_FigS4.png", plot=f.s4, width=12, height=4.8, dpi=400)
```
