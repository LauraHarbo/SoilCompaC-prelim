---
title: "1_ReciprocalDataDriven_RandomForest"
author: "LSH"
date: "2024-07-08"
...
#                                "Data",
#                                 "20240830_allCountries_BaselineModelData_LSH.csv"), row.names=FALSE) # DATE_datadescription_INITIALS
```
...
# Baseline model
```{r}
# test spearman rank coefficient correlation
library(rstatix)

# remove categorical variables
train1_spearman <- train1 %>% select(-c(id, ID, Survey, EUSSubregion, Portr_Petr))

# this piece of code takes a little while to run... 
baseline_corr <- train1_spearman %>% cor_mat(Clay:Vertisols
                            , method = "spearman") %>% cor_gather() %>% 
  mutate(
    X = ifelse(var1 == var2, 0, 1) # remove the diagonal
  )
...
```{r}
set.seed(149)
baselineModel <- ranger(
              BD ~  . -ID -Survey -id, # "-" indicates the columns that should not be included in the model
              data=train1,
              num.trees=1000,
              mtry=floor(sqrt((ncol(train1)-3))), # -3 to account for the three columns that should not be included in the model
...
#                                "Data",
#                                 "20240830_baselinemodel_VarImp_LSH.csv"), row.names=FALSE)
```
...
df.pdp_silt <- pdp::partial(baselineModel, pred.var = c("Silt"))
...
#                                 "Data",
#                                 "20240830_baselinemodel_pdp_oc_LSH.csv"), row.names=FALSE)
# 
# write.csv(df.pdp_silt,file=here::here("Derived",
#                                 "Data",
#                                 "20240830_baselinemodel_pdp_silt_LSH.csv"), row.names=FALSE)
# 
# write.csv(df.pdp_map,file=here::here("Derived",
#                                 "Data",
#                                 "20240830_baselinemodel_pdp_map_LSH.csv"), row.names=FALSE)
# 
# write.csv(df.pdp_clay,file=here::here("Derived",
#                                 "Data",
#                                 "20240830_baselinemodel_pdp_clay_LSH.csv"), row.names=FALSE)
# 
# write.csv(pdp_texture,file=here::here("Derived",
#                                 "Data",
#                                 "20240830_baselinemodel_2sidedpdp_texture_LSH.csv"), row.names=FALSE)
```
...
vars <- df.filtereddata %>%
    ...
    mutate(label = case_when(
                        ...
                        name == "CN" ~ "C:N ratio",
                        # name == "slope" ~ "Slope (unit?)",
                        # name == "wettness" ~ "Wetness (unit?)",
                        # name == "flatness" ~ "Flatness (unit?)",
                        # name == "MAT" ~ "Mean annual temperature (°C)",
                        # name == "MAP" ~ "Mean annual precipitation (mm)",
                        # name == "MeanDailyHighWarmest" ~ "Mean Daily High, warmest month (°C)",
                        # name == "MeanDailyLowColdest" ~ "Mean Daily Low, coldest month (°C)",
                        # name == "PrecipWettest" ~ "Precipitation, wettest month (mm)",
                        # name == "PrecipDriest" ~ "Precipitation, driest month (mm)"
                        ),
...
ggplot(pred, 
  ...
  #theme_minimal(base_size = 20)+
    ...
    #scale_fill_carto_d(palette = "Vidid")+
...
pred %>%
  ...
  summarise(
    n = length(ID),
    meanObs = round(mean(BD), 3), 
    meanPred = round(mean(pred), 3),
    meanBD_diff = round(mean(BD_diff), 3),
    sdDiff = round(sd(BD_diff), 3)
  )
```
...
df.export_all <- merge(df.filtereddata, 
                       pred%>%
                     select(-c(LU, Clay, Silt, BD, Survey #Survey#X_UTM32_Intern, Y_UTM32_Intern
                               )), 
                            by=c("ID"), all=T)#%>%
...
#                                 "Data",
#                                "20240830_SubsoilCompaction_allCountries_LSH.csv"), row.names=FALSE)
```
...
#                                "Data",
#                                 "20241025_posthocModelData_LSH.csv"), row.names=FALSE)
```
...
#                                "Data",
#                                 "20240830_posthocModel_LSH.csv"), row.names=FALSE)
```
...
#                                "Data",
#                                 "20240830_posthoc_2sidedpdp_texture_LSH.csv"), row.names=FALSE)
```
...
#                                "Data",
#                                 "20240830_posthoc_pdp_oc_LSH.csv"), row.names=FALSE)
# 
# write.csv(df.posthocpdp_silt,file=here::here("Derived",
#                                "Data",
#                                 "20240830_posthoc_pdp_silt_LSH.csv"), row.names=FALSE)
# 
# write.csv(df.posthocpdp_map,file=here::here("Derived",
#                                "Data",
#                                 "20240830_posthoc_pdp_map_LSH.csv"), row.names=FALSE)
# 
# write.csv(df.posthocpdp_clay,file=here::here("Derived",
#                                "Data",
#                                 "20240830_posthoc_pdp_clay_LSH.csv"), row.names=FALSE)
# 
# write.csv(df.posthocpdp_MFS,file=here::here("Derived",
#                                "Data",
#                                 "20240830_posthoc_pdp_mfs_LSH.csv"), row.names=FALSE)
# 
# write.csv(df.posthocpdp_MAT,file=here::here("Derived",
#                                "Data",
#                                 "20240830_posthoc_pdp_mat_LSH.csv"), row.names=FALSE)
# 
# write.csv(df.posthocpdp_ph,file=here::here("Derived",
#                                "Data",
#                                 "20240830_posthoc_pdp_ph_LSH.csv"), row.names=FALSE)
# 
# write.csv(df.posthocpdp_till,file=here::here("Derived",
#                                "Data",
#                                 "20240830_posthoc_pdp_till_LSH.csv"), row.names=FALSE)
# 
# write.csv(df.posthocpdp_ps,file=here::here("Derived",
#                                "Data",
#                                 "20240830_posthoc_pdp_ps_LSH.csv"), row.names=FALSE)
```
